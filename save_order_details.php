...
if (!empty($_POST["user_id"])) {
    ...
    $query = "INSERT INTO `uder_order` ( `order_id`, `order_user_id`, `order_user_name`, `order_user_contact`, `order_user_state`, `order_user_city`, `order_user_address`, `order_user_email`, `order_time`, `order_total_amount`,  `order_tax`,  `order_gross_amount`, `order_status`) 
    VALUES ('','$user_id', '$order_user_name', '$order_user_contact', '$order_user_state', '$order_user_city', '$order_user_address', '$order_user_email', '$order_time' , '$all_total_price_post' , '$delivery_chearge' , '$gross_total' , '0');";
    ...
    if (mysqli_query($connection, $query)) {
        ...
        if (@$get_details->num_rows > 0) {
            ...
            while ($row = mysqli_fetch_array($get_details)) {
                ...
                $o_query = "INSERT INTO `uder_order_details` (`uder_order_details`, `uod_order_id`, `uod_item_id`, `uod_item_cat`, `uod_price`, `uod_quantity`, `uod_status`) 
                VALUES ('', '$order_id', '$item_id', '$item_category ', '$item_price', '$cart_qty', '0');";
